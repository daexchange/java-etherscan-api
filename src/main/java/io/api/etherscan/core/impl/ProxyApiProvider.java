...
public class ProxyApiProvider extends BasicProvider implements IProxyApi {
	...
	ProxyApiProvider(final IQueueManager queue, final String baseUrl, final IHttpExecutor executor) {
	...
	public long blockNoLast() throws ApiException {
		...
		return (BasicUtils.isEmpty(response.getResult())) ? -1 : BasicUtils.parseHex(response.getResult()).longValue();
	}
	...
	public Optional<BlockProxy> block(final long blockNo) throws ApiException {
		...
		final String urlParams = ACT_BY_BLOCKNO_PARAM + TAG_PARAM + HEX + Long.toHexString(compBlockNo) + BOOLEAN_PARAM;
	...
	public Optional<BlockProxy> blockUncle(final long blockNo, final long index) throws ApiException {
		...
		final String urlParams = ACT_UNCLE_BY_BLOCKNOINDEX_PARAM + TAG_PARAM + "0x" + Long.toHexString(compBlockNo)
				+ INDEX_PARAM + "0x" + Long.toHexString(compIndex);
		final BlockProxyTO response = getRequest(urlParams, BlockProxyTO.class);
		return Optional.ofNullable(response.getResult());
	}
	...
	public Optional<String> txSendRaw(final String hexEncodedTx) throws ApiException {
		...
		if (response.getError() != null)
			throw new EtherScanException("Error occurred with code " + response.getError().getCode() + " with message "
					+ response.getError().getMessage() + ", error id " + response.getId() + ", jsonRPC "
					+ response.getJsonrpc());
	...
	public Optional<String> storageAt(final String address, final long position) throws ApiException {
		...
		final String urlParams = ACT_STORAGEAT_PARAM + ADDRESS_PARAM + address + POSITION_PARAM + compPosition
				+ TAG_LAST_PARAM;
		final StringProxyTO response = getRequest(urlParams, StringProxyTO.class);
	...
	public BigInteger gasPrice() throws ApiException {
		...
		return (BasicUtils.isEmpty(response.getResult())) ? BigInteger.valueOf(-1)
